...
if __name__ == "__main__":
    ...
    ###############################################
        ...
        # 交叉验证推理模式
            ...
            # 对每个折进行推理
            foldResults = np.zeros((FOLD_NUM, len(test_path)), dtype=int)
            for fold in range(1, FOLD_NUM + 1):
                print('===================fold %d===================' % (fold))
                # 初始化分类器并加载对应折的权重
                classifier = VolumeClassifier(**INIT_TRAINER)
                ...
                if args.save == 'no' or args.save == 'n':
                    # 不保存中间特征
                    result, _, _ = classifier.inference(test_path, label_dict)
                    foldResults[fold - 1, :] = result['pred']
                    print('run time:%.4f' % (time.time() - start_time))
                else:
                    # 保存中间特征
                    result, feature_in, feature_out = classifier.inference(
            ...
            # 计算最终结果
            # 统计每个样本在所有折中的预测结果，取出现次数最多的类别作为最终结果
            final_result = np.apply_along_axis(lambda x: np.bincount(x).argmax(), axis=0, arr=foldResults)
            # 计算准确率
            accuracy = np.sum(final_result == np.array(list(label_dict.values()))) / len(test_path)
