...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x.shape", x.shape, "self.position_embeddings.shape", self.position_embeddings.shape)
...
class Encoder(nn.Module):
    def __init__(self, config,flag:bool = True):
        super(Encoder, self).__init__()
        self.layer = nn.ModuleList()
        for _ in range(config.transformer["num_layers"] - 1):
            layer = Block(config)
            self.layer.append(copy.deepcopy(layer))
        self.ifSelectPart = flag
        if self.ifSelectPart:
    ...
    def forward(self, hidden_states):
        # 如果不使用part select，就直接使用hidden_states to last layer
        if not self.ifSelectPart:
            for layer in self.layer:
                hidden_states, weights = layer(hidden_states)
            part_states = self.part_layer(hidden_states)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, ifSelectPart: bool = True):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size)
        self.encoder = Encoder(config, ifSelectPart)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, ifSelectPart: bool = True, ifConLoss: bool = True, zero_head=False):
        super(VisionTransformer, self).__init__()
        self.num_classes = num_classes
        self.smoothing_value = smoothing_value
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size, ifSelectPart)
        self.part_head = Linear(config.hidden_size, num_classes)
        self.ifConLoss = ifConLoss
