...
class VolumeClassifier(object):
    ...
    def __init__(self,
        ...
        # 输出参数设置到控制台
        print('net_name:{}'.format(self.net_name), 
              'lr:{}'.format(self.lr),
              'n_epoch:{}'.format(self.n_epoch),
              'num_classes:{}'.format(self.num_classes),
              'image_size:{}'.format(self.image_size),
              'batch_size:{}'.format(self.batch_size),
              'num_workers:{}'.format(self.num_workers),
              'device:{}'.format(self.device),
              'pre_trained:{}'.format(self.pre_trained),
              'weight_decay:{}'.format(self.weight_decay),
              'momentum:{}'.format(self.momentum),
              'gamma:{}'.format(self.gamma),
              'milestones:{}'.format(self.milestones),
              'T_max:{}'.format(self.T_max),
              'use_fp16:{}'.format(self.use_fp16),
              'dropout:{}'.format(self.dropout),
              'warmup_epochs:{}'.format(self.warmup_epochs),
    ...
    def trainer(self,
        ...
        print('optimizer:{}'.format(optimizer),
              'loss_fun:{}'.format(loss_fun),
              'class_weight:{}'.format(class_weight),
              'lr_scheduler:{}'.format(lr_scheduler))
        # 设置随机种子以确保可重复性
        torch.manual_seed(0)
        np.random.seed(0)
    ...
    def _get_optimizer(self, optimizer, net, lr):
        ...
        return optimizer
    # 获取学习率调度器的函数
    def _get_lr_scheduler(self, lr_scheduler_name, optimizer):
            main_scheduler = None
            # Create the main scheduler instance based on lr_scheduler_name
            if lr_scheduler_name == 'ReduceLROnPlateau':
                # IMPORTANT: Using ReduceLROnPlateau with SequentialLR for warmup is complex.
                # SequentialLR.step() does not pass arguments to the underlying scheduler's step.
                # ReduceLROnPlateau requires a metric for its step method (e.g., lr_scheduler.step(val_loss)).
                # This will not work correctly if lr_scheduler.step() is called without arguments in the training loop
                # when ReduceLROnPlateau is active within SequentialLR.
                # Consider manual warmup or ensure your training loop handles this specific case.
                main_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                    optimizer, mode='min', patience=5, verbose=True)
            elif lr_scheduler_name == 'MultiStepLR':
                # Adjust milestones for the main scheduler phase, occurring after warmup
                adjusted_milestones = [m - self.warmup_epochs for m in self.milestones if m > self.warmup_epochs]
                main_scheduler = torch.optim.lr_scheduler.MultiStepLR(
                    optimizer, milestones=adjusted_milestones, gamma=self.gamma)
            elif lr_scheduler_name == 'CosineAnnealingLR':
                # Adjust T_max for the main scheduler phase
                adjusted_T_max = self.T_max - self.warmup_epochs
                if adjusted_T_max <= 0:
                    adjusted_T_max = 1 # Ensure T_max is at least 1
                main_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    optimizer, T_max=adjusted_T_max)
            elif lr_scheduler_name == 'CosineAnnealingWarmRestarts':
                # Note: CosineAnnealingWarmRestarts has its own restart/warmup logic.
                # An additional initial warmup might interact in complex ways.
                main_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                    optimizer, T_0=20, T_mult=2) # Using original T_0=20
            else:
                raise ValueError(f"Unsupported lr_scheduler: {lr_scheduler_name}")

            # If warmup_epochs is specified and greater than 0, prepend a LinearLR warmup scheduler
            if self.warmup_epochs > 0:
                warmup_scheduler = torch.optim.lr_scheduler.LinearLR(
                    optimizer,
                    start_factor=0.1,  # Start LR will be 0.1 * base_lr
                    end_factor=1.0,    # End LR of warmup will be 1.0 * base_lr
                    total_iters=self.warmup_epochs  # Number of epochs for warmup
                )
                
                # Chain the warmup_scheduler and the main_scheduler
                combined_scheduler = torch.optim.lr_scheduler.SequentialLR(
                    optimizer,
                    schedulers=[warmup_scheduler, main_scheduler],
                    milestones=[self.warmup_epochs]  # Switch to main_scheduler after self.warmup_epochs
                )
                return combined_scheduler
            else:
                # No warmup, return the main scheduler directly
